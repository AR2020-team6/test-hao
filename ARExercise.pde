...
PShape blueEyes; /// prepare the variable for model 
...
// 0x1228 most right
// 0x1c44  most left
final int[] towardsList = {0x005a,0x1c44,0x0272,0x0b44,0x1228,0x0690}; /// [Yuzhe] [?] How to modifiy this?
/// [Yuzhe] [?] I think this `towardList` array is very important
...
void draw() {
  ...
  /// [Yuzhe] marker visualization (up-left-side's detected markers)
  if (!USE_SAMPLE_IMAGE) {
    if (USE_DIRECTSHOW) {
  ...
  /// if make this part comments, the model will disappear
  for (int i = 0; i < markers.size(); i++) {
    Marker m = markers.get(i);
    // println("marker_code: ", m.code);
    // println("marker_pose: ",m.pose);
    markerPoseMap.put(m.code, m.pose);
  }

  /// [Yuzhe] multi-dragons testy
  for (int i = 0; i < towardsList.length; i++) {
    ...
    // }
      ...
      ///// [?] what is this part for? after make this part comment, the code is still working
      // noFill();
      // strokeWeight(3);
      // stroke(255, 0, 0);
  ...
  //   /// [Yuzhe] build the snowman model and ball animation
  //   pushMatrix();
